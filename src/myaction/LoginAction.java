...
public class LoginAction extends ActionSupport{
	...
	public String execute() throws Exception{
		...
		//如果已经注册则开始判断同一用户名的密码是否正确
			...
			if(rightPassword){//如果密码正确，开始检查验证码输入是否正确
				...
				if(rand.equals(code))
				{
					...
					Integer s=user.getUserType();
					if(s==3)
						return LOGIN;//登陆的是超级管理员
					if(s==2)
					{
						ActionContext.getContext().getSession().put("ModuleAdmin", user.getUserName());
						HttpServletRequest request2=ServletActionContext.getRequest();
						String ModuleAdmin=(String) request2.getSession().getAttribute("ModuleAdmin");
						System.out.println("登陆的是模块管理员，Myname="+ModuleAdmin);
						return NONE;
					}
					else
					{
